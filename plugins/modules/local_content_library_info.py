...
DOCUMENTATION = r'''
---
module: local_content_library_info
short_description: Gathers information about local content libraries
description:
    - Gathers information about one or more local content libraries.
    - You can search for libraries by name or retrieve all local libraries.
author:
...
options:
    name:
        description:
            - The name of the local content library to gather information about.
            - If not provided, all local content libraries will be returned.
        type: str
        required: false
        aliases: [library_name]
    library_id:
        description:
            - The ID of the local content library to gather information about.
...
EXAMPLES = r'''
- name: Gather information about all local content libraries
  vmware.vmware.local_content_library_info:
...
libraries:
    description:
        - Dictionary of local content libraries found.
    ...
    sample: {
        "12345678-1234-1234-1234-123456789012": {
            ...
            "name": "my-library",
            "description": "My local content library",
            "type": "LOCAL",
            ...
            "publish_info": {
                ...
                "publish_url": "https://vcenter.example.com/..."
            },
            "version": "1"
        }
    }
...
class LocalContentLibraryInfo(ModuleRestBase):
    def __init__(self, module):
        super(LocalContentLibraryInfo, self).__init__(module)

    def gather_libraries_info(self):
        """
        Gather information about local content libraries based on the search parameters
        """
        all_libraries_info = {}
        ...
        if self.params.get('library_id'):
            try:
                library = self.api_client.content.LocalLibrary.get(self.params['library_id'])
                if library.type == LibraryModel.LibraryType.LOCAL:
        ...
        library_ids = self.api_client.content.LocalLibrary.list()
        for library_id in library_ids:
            library = self.api_client.content.LocalLibrary.get(library_id)
    ...
    def _library_to_dict(self, library):
        ...
        if library.publish_info:
            library_dict['publish_info'] = {
                'published': library.publish_info.published,
                'authentication_method': library.publish_info.authentication_method,
                'persist_json_enabled': library.publish_info.persist_json_enabled,
                'publish_url': library.publish_info.publish_url if hasattr(library.publish_info, 'publish_url') else None
            }
...
def main():
    ...
    library_info = LocalContentLibraryInfo(module)
